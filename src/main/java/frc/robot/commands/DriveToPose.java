...
public class DriveToPose extends Command {
  ...
  TuneableProfiledPID angleController =
      new TuneableProfiledPID("angleController", 4.5, 0.0, 0.4, 20, 20.0);

  TuneableProfiledPID xController = new TuneableProfiledPID("xController", 5.3, 0.0, 0.2, 3.7, 4);

  TuneableProfiledPID yController = new TuneableProfiledPID("yController", 5.3, 0.0, 0.2, 3.7, 4);
  ...
  public void execute() {
    ...
    Translation2d offsetVector =
        new Translation2d(
            xController.calculate(relativePose2d.getX()),
            yController.calculate(relativePose2d.getY()));

    // Calculate total linear velocity
    Translation2d linearVelocity = offsetVector.rotateBy(targetRotation2d);

    // Calculate angular speed
    double omega =
        angleController.calculate(drive.getRotation().getRadians(), targetRotation2d.getRadians());
